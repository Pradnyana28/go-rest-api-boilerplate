...
import (
	...
	"github.com/Pradnyana28/go-rest-api-boilerplate/pkg/api/auth/transport"
...
func TestLogin(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantResp: &rest.AuthToken{Token: "jwttokenstring", Expires: mock.TestTime(2018).Format(time.RFC3339), RefreshToken: "refreshtoken"},
		},
	}
...
func TestRefresh(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantResp: &rest.RefreshToken{Token: "jwttokenstring", Expires: mock.TestTime(2018).Format(time.RFC3339)},
		},
	}
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer res.Body.Close()
			if tt.wantResp != nil {
				response := new(rest.RefreshToken)
				if err := json.NewDecoder(res.Body).Decode(response); err != nil {
					t.Fatal(err)
				}
				assert.Equal(t, tt.wantResp, response)
...
func TestMe(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantStatus: http.StatusOK,
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, i int) (*rest.User, error) {
					return &rest.User{
						Base: rest.Base{
							ID: i,
						},
